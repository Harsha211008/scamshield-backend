...
class RuleAnalyzer:
    ...
    def evaluate(self, text, entities=None, patterns=None):
        ...
        # ---------------------------------
        # Delivery OTP (legitimate context)
        # ---------------------------------
        if OTP_PATTERN.search(text) and any(w in text for w in DELIVERY_WORDS):

            # Only treat as delivery OTP if reward words NOT present
            if not any(w in text for w in REWARD_WORDS):
                rules.append("delivery_otp")

        # ---------------------------------
        # Reward + OTP (high risk scam)
        # ---------------------------------
